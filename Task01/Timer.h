...
private:
	...
	__int64 m_currentTime = 0;					// 현재 시간
	__int64 m_lastTime = 0;						// 이전 시간
	__int64 m_perfFreq = 0;						// 주파수 . QueryPerformanceFrequency .

	FLOAT m_frameTime[MAX_COUNT] = { 0, };		// 초당 프레임 수
	ULONG m_sampleCount = 0;					// 현재 시간 샘플 수

	ULONG m_frameRate = 0;						// Tick Func 호출 횟수
	ULONG m_fpsFrameCount = 0;					// 해당 초 동안 발생한 총 프레임 수. -> m_frameRate 에 저장.
